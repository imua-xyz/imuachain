	...
	testcases := []struct {
	...
	}{
		{
			...
			runFn: func() {
				...
				mintParam := suite.App.ImmintKeeper.GetParams(suite.Ctx)
				epochRewardDec := sdk.NewDecFromInt(mintParam.EpochReward)
				expectedAVSRewards, _ := suite.calculateExpectedOperatorReward(
					operatorInitialPower, totalPower,
		...
		},
		{
			...
			runFn: func() {
				...
				mintParam := suite.App.ImmintKeeper.GetParams(suite.Ctx)
				epochRewardDec := sdk.NewDecFromInt(mintParam.EpochReward)
				stakingRewardsEpoch1, _ := suite.calculateExpectedOperatorReward(
					operatorInitialPower, totalPower,
					operatorInitialPower, epochRewardDec,
					feedistributiontypes.DefaultParams().CommunityTax, testutil.DefaultOperatorCommission.Rate,
					1, suite.DogfoodAVSAddr, utils.BaseDenom)
				...
				rewardsEpoch2, _ := suite.calculateExpectedOperatorReward(
					operatorCurPower, totalPower,
					operatorCurPower, epochRewardDec,
					feedistributiontypes.DefaultParams().CommunityTax, testutil.DefaultOperatorCommission.Rate,
					1, suite.DogfoodAVSAddr, utils.BaseDenom)
				// calculate the expected rewards for staking assets and compounding rewards
				stakingRewardsEpoch2 := rewardsEpoch2.Rewards.MulDecTruncate(operatorInitialPower.QuoTruncate(operatorInitialPower.Add(compoundingPower)))
				compoundingRewardsEpoch2 := rewardsEpoch2.Rewards.MulDecTruncate(compoundingPower.QuoTruncate(operatorInitialPower.Add(compoundingPower)))

				// get the unclaimed rewards of test operator
				operatorUnclaimedRewards, err := suite.App.DistrKeeper.GetOperatorUnclaimedRewards(suite.Ctx, suite.testOperators[0].String(), suite.DogfoodAVSAddr)
				suite.Require().NoError(err)
				expectedOutstandingRewards := stakingRewardsEpoch1.Rewards.Add(stakingRewardsEpoch2...)
				...
				expectedCompoundingRewards := []feedistributiontypes.CompoundingRewardsPerAsset{
					{
						...
						Rewards: feedistributiontypes.NewCommonAVSRewards(feedistributiontypes.CommonAVSRewardData{
							AVSAddress: suite.DogfoodAVSAddr,
							Rewards:    compoundingRewardsEpoch2,
						}),
					},
				}
		...
		},
		{
			...
			runFn: func() {
				...
				stakingRewardsEpoch2 := rewardsEpoch2.Rewards.MulDecTruncate(operatorInitialPower.QuoTruncate(operatorInitialPower.Add(compoundingPower)))
				totalStakingRewards := stakingRewardsEpoch1.Rewards.Add(stakingRewardsEpoch2...)
				compoundingRewardsEpoch2 := rewardsEpoch2.Rewards.MulDecTruncate(compoundingPower.QuoTruncate(operatorInitialPower.Add(compoundingPower)))
				...
				stakerCompoundingRewards := compoundingRewardsEpoch2.MulDecTruncate(stakerStakingRewards[0].Amount.QuoTruncate(totalStakingRewards[0].Amount))
				...
				expectedCompoundingRewards := []feedistributiontypes.CompoundingRewardsPerAsset{
					{
						...
						Rewards: feedistributiontypes.NewCommonAVSRewards(feedistributiontypes.CommonAVSRewardData{
							AVSAddress: suite.DogfoodAVSAddr,
							Rewards:    compoundingRewardsEpoch2.Sub(stakerCompoundingRewards),
						}),
					},
				}
				suite.Require().Equal(expectedCompoundingRewards, operatorUnclaimedRewards.RewardsFromCompounding)
