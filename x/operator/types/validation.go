...
func (info *OperatorInfo) ValidateBasic() error {
	...
	// basic check 2
	_, err := sdk.AccAddressFromBech32(info.OperatorAddr)
	if err != nil {
		return errorsmod.Wrap(
	...
	if info.Description.Moniker == "" {
		return errorsmod.Wrap(
			ErrParameterInvalid, "ValidateBasic: operator moniker is empty",
		)
	}

	// do not allow operator description to exceed the maximum length
	_, err = info.Description.EnsureLength()
	if err != nil {
		return err
	}
