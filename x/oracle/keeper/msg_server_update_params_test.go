...
var _ = Describe("MsgUpdateParams", Ordered, func() {
	...
	fmt.Println("authAddrString2", authAddrString)
	...
	Context("Update Chains", func() {
		...
		It("add chain with new name", func() {
			msg := types.NewMsgUpdateParams(authAddrString, inputAddChains[0])
		...
		It("add chain with duplicated name", func() {
			_, err := ks.ms.UpdateParams(ks.ctx.WithChainID(chainIDtest), types.NewMsgUpdateParams(authAddrString, inputAddChains[1]))
	...
	Context("Update Sources", func() {
		...
		It("add valid source with new name", func() {
			_, err := ks.ms.UpdateParams(ks.ctx.WithChainID(chainIDtest), types.NewMsgUpdateParams(authAddrString, inputAddSources[0]))
			Expect(err).Should(BeNil())
			p := ks.k.GetParams(ks.ctx)
			Expect(p.Sources[2].Name).Should(BeEquivalentTo("CoinGecko"))
		})
		It("add invalid source with new name", func() {
			_, err := ks.ms.UpdateParams(ks.ctx.WithChainID(chainIDtest), types.NewMsgUpdateParams(authAddrString, inputAddSources[1]))
		...
		It("add source with duplicated name", func() {
			_, err := ks.ms.UpdateParams(ks.ctx.WithChainID(chainIDtest), types.NewMsgUpdateParams(authAddrString, inputAddSources[2]))
			Expect(err).Should(MatchError(types.ErrInvalidParams.Wrap("invalid source to add, duplicated")))
		})
	})
	Context("Update Tokens", func() {
		...
		for i, input := range inputUpdateTokens {
			It("", func() {
				...
				_, err := ks.ms.UpdateParams(ks.ctx.WithChainID(chainIDtest), types.NewMsgUpdateParams(authAddrString, input))
				if errs[i] == nil {
					Expect(err).Should(BeNil())
				} else {
